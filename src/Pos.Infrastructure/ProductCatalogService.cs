...
public sealed record ProductCommand(string Code, string Description, decimal Price, decimal Cost = 0m, decimal WholesalePrice = 0m, decimal WholesaleMinimumQuantity = 0m, bool IsKit = false, string UnitOfMeasure = "Pieza");
public sealed record ProductResult(Guid Id, string Code, string Description, decimal Price, decimal Cost, decimal WholesalePrice, decimal WholesaleMinimumQuantity, bool IsKit, string UnitOfMeasure, bool IsActive);
...
public sealed class ProductCatalogService(PosDbContext database)
{
    public async Task<ProductResult?> CreateAsync(string accessToken, ProductCommand command, CancellationToken cancellationToken)
    {
        ...
        var product = new ProductRecord { Id = Guid.NewGuid(), Code = command.Code.Trim(), NormalizedCode = normalizedCode, Description = command.Description.Trim(), Price = decimal.Round(command.Price, 2), Cost = decimal.Round(command.Cost, 2), WholesalePrice = decimal.Round(command.WholesalePrice, 2), WholesaleMinimumQuantity = decimal.Round(command.WholesaleMinimumQuantity, 3), UnitOfMeasure = NormalizeUnit(command.UnitOfMeasure), IsKit = command.IsKit, IsActive = true };
    ...
    public async Task<ProductResult?> UpdateAsync(string accessToken, Guid id, ProductCommand command, CancellationToken cancellationToken)
    {
        ...
        product.Code = command.Code.Trim(); product.NormalizedCode = normalizedCode; product.Description = command.Description.Trim(); product.Price = decimal.Round(command.Price, 2); product.Cost = decimal.Round(command.Cost, 2); product.WholesalePrice = decimal.Round(command.WholesalePrice, 2); product.WholesaleMinimumQuantity = decimal.Round(command.WholesaleMinimumQuantity, 3); product.UnitOfMeasure = NormalizeUnit(command.UnitOfMeasure); product.IsKit = command.IsKit;
        await database.SaveChangesAsync(cancellationToken);
        return ToResult(product);
    }
    ...
    private static ProductResult ToResult(ProductRecord product) => new(product.Id, product.Code, product.Description, product.Price, product.Cost, product.WholesalePrice, product.WholesaleMinimumQuantity, product.IsKit, product.UnitOfMeasure, product.IsActive);
    private static string NormalizeUnit(string value) => string.IsNullOrWhiteSpace(value) ? "Pieza" : value.Trim();
