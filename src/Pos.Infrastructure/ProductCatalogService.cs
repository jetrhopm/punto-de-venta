...
public sealed class ProductCatalogService(PosDbContext database)
{
    public async Task<ProductResult?> CreateAsync(string accessToken, ProductCommand command, CancellationToken cancellationToken)
    {
        ...
        if (product.Stock > 0m && store.InventoryEnabled)
        {
            database.InventoryMovements.Add(new InventoryMovementRecord { Id = Guid.NewGuid(), ProductId = product.Id, UserId = userId.Value, OperationId = Guid.NewGuid(), Quantity = product.Stock, StockBefore = 0m, StockAfter = product.Stock, Reason = "InitialStock", CreatedAtUtc = DateTimeOffset.UtcNow });
        }
    ...
    private static void Validate(ProductCommand command)
    {
        ...
        if (command.Price < 0m || command.Price > 9999999999999999.99m || command.Cost < 0m || command.Cost > 9999999999999999.99m || command.WholesalePrice < 0m || command.WholesaleMinimumQuantity < 0m || command.InitialStock < 0m || command.MinimumStock < 0m || command.MaximumStock < 0m || command.MaximumStock > 0m && command.MaximumStock < command.MinimumStock || command.ProfitPercent is < 0m or > 1000m || command.WholesaleProfitPercent is < 0m or > 1000m || (command.WholesalePrice > 0m && command.WholesaleMinimumQuantity <= 0m)) throw new ArgumentException("Los precios, existencias, límites y condiciones de mayoreo no son válidos.");
    }
}
