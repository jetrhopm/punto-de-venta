...
public sealed class ProductCatalogService(PosDbContext database)
{
    public async Task<ProductResult?> CreateAsync(string accessToken, ProductCommand command, CancellationToken cancellationToken)
    {
        ...
        var product = CreateRecord(command, normalizedCode, store);
    ...
    public async Task<ProductResult?> UpdateAsync(string accessToken, Guid id, ProductCommand command, CancellationToken cancellationToken)
    {
        ...
        await ValidateDepartmentAsync(command.DepartmentId, cancellationToken);
        var store = await database.Stores.OrderBy(item => item.CreatedAtUtc).FirstAsync(cancellationToken);
        var minimumStock = command.MinimumStock ?? product.MinimumStock;
        var maximumStock = command.MaximumStock ?? product.MaximumStock;
        if (maximumStock > 0m && maximumStock < minimumStock) throw new ArgumentException("El inventario máximo no puede ser menor al mínimo.");
        product.Code = command.Code.Trim(); product.NormalizedCode = normalizedCode; product.Description = command.Description.Trim(); product.Cost = decimal.Round(command.Cost, 2); product.ProfitPercent = decimal.Round(command.ProfitPercent, 2); product.Price = ResolvePrice(command.Price, product.Cost, product.ProfitPercent, store); product.WholesaleProfitPercent = decimal.Round(command.WholesaleProfitPercent, 2); product.WholesalePrice = ResolveWholesalePrice(command.WholesalePrice, product.Cost, product.WholesaleProfitPercent, store); product.WholesaleMinimumQuantity = decimal.Round(command.WholesaleMinimumQuantity, 3); product.UnitOfMeasure = NormalizeUnit(command.UnitOfMeasure); product.DepartmentId = command.DepartmentId; product.IsKit = command.IsKit; product.IsCommonProduct = command.IsCommonProduct;
        if (product.MinimumStock != minimumStock || product.MaximumStock != maximumStock)
        {
            database.InventoryLimitChanges.Add(new InventoryLimitChangeRecord
            {
                Id = Guid.NewGuid(),
                ProductId = product.Id,
                UserId = userId.Value,
                ...
                PreviousMaximumStock = product.MaximumStock,
                MinimumStock = decimal.Round(minimumStock, 3),
                MaximumStock = decimal.Round(maximumStock, 3),
                CreatedAtUtc = DateTimeOffset.UtcNow
            });
            product.MinimumStock = decimal.Round(minimumStock, 3);
            product.MaximumStock = decimal.Round(maximumStock, 3);
        }
        await database.SaveChangesAsync(cancellationToken);
        return ToResult(product);
    }
    ...
    private ProductRecord CreateRecord(ProductCommand command, string normalizedCode, StoreRecord store) => new() { Id = Guid.NewGuid(), Code = command.Code.Trim(), NormalizedCode = normalizedCode, Description = command.Description.Trim(), Price = ResolvePrice(command.Price, command.Cost, command.ProfitPercent, store), Cost = decimal.Round(command.Cost, 2), ProfitPercent = decimal.Round(command.ProfitPercent, 2), WholesalePrice = ResolveWholesalePrice(command.WholesalePrice, command.Cost, command.WholesaleProfitPercent, store), WholesaleProfitPercent = decimal.Round(command.WholesaleProfitPercent, 2), WholesaleMinimumQuantity = decimal.Round(command.WholesaleMinimumQuantity, 3), Stock = decimal.Round(command.InitialStock, 3), MinimumStock = decimal.Round(command.MinimumStock ?? 0m, 3), MaximumStock = decimal.Round(command.MaximumStock ?? 0m, 3), DepartmentId = command.DepartmentId, UnitOfMeasure = NormalizeUnit(command.UnitOfMeasure), IsKit = command.IsKit, IsCommonProduct = command.IsCommonProduct, IsActive = true };
    ...
    private static void Validate(ProductCommand command)
    {
        ...
        var minimumStock = command.MinimumStock ?? 0m;
        var maximumStock = command.MaximumStock ?? 0m;
        if (command.Price < 0m || command.Price > 9999999999999999.99m || command.Cost < 0m || command.Cost > 9999999999999999.99m || command.WholesalePrice < 0m || command.WholesaleMinimumQuantity < 0m || command.InitialStock < 0m || minimumStock < 0m || maximumStock < 0m || (maximumStock > 0m && maximumStock < minimumStock) || command.ProfitPercent is < 0m or > 1000m || command.WholesaleProfitPercent is < 0m or > 1000m || (command.WholesalePrice > 0m && command.WholesaleMinimumQuantity <= 0m)) throw new ArgumentException("Los precios, existencias, límites y cantidades de mayoreo no son válidos.");
    }
}
