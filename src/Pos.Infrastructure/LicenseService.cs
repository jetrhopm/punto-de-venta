...
public sealed class LicenseService(PosDbContext database)
{
    ...
    public async Task<LicenseStatusResult?> ImportAsync(string token, ImportLicenseCommand command, CancellationToken cancellationToken)
    {
        ...
        return ImportForLocalActivation(command);
    }

    // La autorización de esta operación es el propio archivo: está firmado y vinculado a una sola máquina.
    // Se usa antes del inicio de sesión para recuperar una instalación cuya demo ya venció.
    public LicenseStatusResult ImportForLocalActivation(ImportLicenseCommand command)
    {
        if (string.IsNullOrWhiteSpace(command.Content)) throw new ArgumentException("Selecciona un archivo de licencia válido.");
