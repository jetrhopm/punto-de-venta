...
public sealed class UserAdministrationService(PosDbContext database, PasswordHasher<UserRecord> passwordHasher)
{
    ...
    {
        if (!await IsAdministratorAsync(token, cancellationToken)) return null;
    ...
    private async Task<bool> IsAdministratorAsync(string token, CancellationToken cancellationToken)
    {
        ...
        return session is not null && await database.Users.AnyAsync(item => item.Id == session.UserId && item.IsActive && item.IsAdministrator, cancellationToken);
    }
