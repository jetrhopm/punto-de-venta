...
public sealed class PosDbContext(DbContextOptions<PosDbContext> options) : DbContext(options)
{
    ...
    public DbSet<MercadoPagoRefundRecord> MercadoPagoRefunds => Set<MercadoPagoRefundRecord>();
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<StoreRecord>(entity =>
        {
            ...
            entity.Property(store => store.BlockSalesWhenCashLimitReached).HasDefaultValue(false);
            ...
            entity.Property(store => store.FacturamaEnabled).HasDefaultValue(false);
            entity.Property(store => store.FacturamaEnvironment).HasMaxLength(20).HasDefaultValue("Sandbox");
            entity.Property(store => store.FacturamaAccountEmail).HasMaxLength(200).HasDefaultValue("");
            entity.Property(store => store.FacturamaPasswordProtected).HasMaxLength(4096).HasDefaultValue("");
            entity.Property(store => store.FacturamaLastVerifiedAtUtc).HasColumnType("timestamp with time zone");
            entity.Property(store => store.FacturamaLastVerificationMessage).HasMaxLength(300).HasDefaultValue("");
        });
        ...
        modelBuilder.Entity<RegisterRecord>(entity =>
        {
            ...
            entity.Property(register => register.CashPaymentEnabled).HasDefaultValue(true);
            entity.Property(register => register.CardPaymentEnabled).HasDefaultValue(true);
            entity.Property(register => register.TransferPaymentEnabled).HasDefaultValue(true);
            entity.Property(register => register.CreditPaymentEnabled).HasDefaultValue(true);
            entity.Property(register => register.MercadoPagoEnabled).HasDefaultValue(false);
        ...
        {
            entity.ToTable("pairing_code"); entity.HasKey(item => item.Id); entity.Property(item => item.CodeHash).HasMaxLength(64).IsRequired(); entity.Property(item => item.CreatedAtUtc).HasColumnType("timestamp with time zone"); entity.Property(item => item.ExpiresAtUtc).HasColumnType("timestamp with time zone"); entity.Property(item => item.UsedAtUtc).HasColumnType("timestamp with time zone"); entity.HasIndex(item => item.CodeHash).IsUnique(); entity.HasIndex(item => item.RegisterId); entity.HasOne<StoreRecord>().WithMany().HasForeignKey(item => item.StoreId).OnDelete(DeleteBehavior.Restrict); entity.HasOne<UserRecord>().WithMany().HasForeignKey(item => item.CreatedByUserId).OnDelete(DeleteBehavior.Restrict); entity.HasOne<RegisterRecord>().WithMany().HasForeignKey(item => item.RegisterId).OnDelete(DeleteBehavior.Restrict);
        });
        modelBuilder.Entity<MercadoPagoOrderRecord>(entity =>
        {
            entity.ToTable("mercado_pago_order"); entity.HasKey(item => item.Id); entity.HasIndex(item => item.OperationId).IsUnique(); entity.HasIndex(item => item.SaleOperationId); entity.HasIndex(item => item.ProviderOrderId).IsUnique();
        ...
        modelBuilder.Entity<MercadoPagoRefundRecord>(entity =>
        {
        ...
        {
            entity.ToTable("mercado_pago_webhook"); entity.HasKey(item => item.Id); entity.HasIndex(item => item.EventKey).IsUnique(); entity.HasIndex(item => new { item.Status, item.NextAttemptAtUtc });
            entity.Property(item => item.EventKey).HasMaxLength(64).IsRequired(); entity.Property(item => item.ProviderOrderId).HasMaxLength(80).IsRequired(); entity.Property(item => item.RequestId).HasMaxLength(160); entity.Property(item => item.Action).HasMaxLength(80); entity.Property(item => item.Status).HasMaxLength(30).IsRequired(); entity.Property(item => item.LastError).HasMaxLength(500); entity.Property(item => item.ReceivedAtUtc).HasColumnType("timestamp with time zone"); entity.Property(item => item.NextAttemptAtUtc).HasColumnType("timestamp with time zone"); entity.Property(item => item.ProcessedAtUtc).HasColumnType("timestamp with time zone");
        });
        modelBuilder.Entity<ProductRecord>(entity =>
        {
            ...
            entity.Property(product => product.IsTemporary).HasDefaultValue(false);
        ...
        modelBuilder.Entity<SessionRecord>(entity =>
        {
            ...
            entity.HasIndex(session => new { session.UserId, session.RegisterId });
            entity.HasOne<RegisterRecord>().WithMany().HasForeignKey(session => session.RegisterId).OnDelete(DeleteBehavior.Restrict);
            entity.HasOne<DeviceRecord>().WithMany().HasForeignKey(session => session.DeviceId).OnDelete(DeleteBehavior.Restrict);
        });
...
public sealed class StoreRecord { public Guid Id { get; set; } public string Name { get; set; } = string.Empty; public string BusinessType { get; set; } = string.Empty; public string LegalName { get; set; } = string.Empty; public string TaxId { get; set; } = string.Empty; public string Address { get; set; } = string.Empty; public string Phone { get; set; } = string.Empty; public string TimeZoneId { get; set; } = "America/Mexico_City"; public string TicketHeader { get; set; } = string.Empty; public string TicketFooter { get; set; } = "Gracias por su compra"; public long NextSaleFolio { get; set; } = 1; public string DefaultWeightUnit { get; set; } = "Kilogramo"; public string CurrencySymbol { get; set; } = "$"; public bool CashPaymentEnabled { get; set; } = true; public bool CardPaymentEnabled { get; set; } = true; public bool TransferPaymentEnabled { get; set; } = true; public bool CreditPaymentEnabled { get; set; } = true; public bool RequireCashCountOnClose { get; set; } = true; public bool AutoAdjustCashDifference { get; set; } = true; public bool CashLimitEnabled { get; set; } public bool BlockSalesWhenCashLimitReached { get; set; } public decimal CashLimit { get; set; } public string CashLimitMessage { get; set; } = "Realiza un retiro de efectivo (F8); se superó el límite permitido en caja."; public bool InventoryEnabled { get; set; } = true; public string InventoryCostMethod { get; set; } = "WeightedAverage"; public bool CreditSalesEnabled { get; set; } = true; public bool CommonProductsEnabled { get; set; } = true; public bool AutoPriceWithProfit { get; set; } = true; public decimal DefaultProfitPercent { get; set; } = 20m; public bool RoundSaleAmounts { get; set; } public string RoundingMode { get; set; } = "Tenths"; public string OccasionalNotice { get; set; } = string.Empty; public int OccasionalNoticeEverySales { get; set; } = 5; public bool MercadoPagoEnabled { get; set; } public string MercadoPagoEnvironment { get; set; } = "Test"; public string MercadoPagoAccessTokenProtected { get; set; } = string.Empty; public string MercadoPagoRefreshTokenProtected { get; set; } = string.Empty; public long? MercadoPagoUserId { get; set; } public DateTimeOffset? MercadoPagoTokenExpiresAtUtc { get; set; } public string MercadoPagoOAuthState { get; set; } = string.Empty; public string MercadoPagoOAuthVerifierProtected { get; set; } = string.Empty; public DateTimeOffset? MercadoPagoOAuthStateExpiresAtUtc { get; set; } public bool FacturamaEnabled { get; set; } public string FacturamaEnvironment { get; set; } = "Sandbox"; public string FacturamaAccountEmail { get; set; } = string.Empty; public string FacturamaPasswordProtected { get; set; } = string.Empty; public DateTimeOffset? FacturamaLastVerifiedAtUtc { get; set; } public string FacturamaLastVerificationMessage { get; set; } = string.Empty; public DateTimeOffset CreatedAtUtc { get; set; } }
...
public sealed class RegisterRecord { public Guid Id { get; set; } public Guid StoreId { get; set; } public string Name { get; set; } = string.Empty; public bool IsActive { get; set; } public bool CashPaymentEnabled { get; set; } = true; public bool CardPaymentEnabled { get; set; } = true; public bool TransferPaymentEnabled { get; set; } = true; public bool CreditPaymentEnabled { get; set; } = true; public bool MercadoPagoEnabled { get; set; } public string MercadoPagoTerminalId { get; set; } = string.Empty; public string MercadoPagoTerminalLabel { get; set; } = string.Empty; }
public sealed class MercadoPagoOrderRecord { public Guid Id { get; set; } public Guid StoreId { get; set; } public Guid RegisterId { get; set; } public Guid OperationId { get; set; } public Guid SaleOperationId { get; set; } public string? ProviderOrderId { get; set; } public string? ProviderPaymentId { get; set; } public decimal Amount { get; set; } public string Status { get; set; } = "Pending"; public string StatusDetail { get; set; } = string.Empty; public DateTimeOffset CreatedAtUtc { get; set; } public DateTimeOffset UpdatedAtUtc { get; set; } }
public sealed class MercadoPagoRefundRecord { public Guid Id { get; set; } public Guid MercadoPagoOrderId { get; set; } public Guid SaleId { get; set; } public Guid OperationId { get; set; } public decimal Amount { get; set; } public string? ProviderRefundId { get; set; } public string Status { get; set; } = "Pending"; public string StatusDetail { get; set; } = string.Empty; public DateTimeOffset CreatedAtUtc { get; set; } public DateTimeOffset UpdatedAtUtc { get; set; } }
public sealed class MercadoPagoWebhookRecord { public Guid Id { get; set; } public string EventKey { get; set; } = string.Empty; public string ProviderOrderId { get; set; } = string.Empty; public string RequestId { get; set; } = string.Empty; public string Action { get; set; } = string.Empty; public string Status { get; set; } = "Received"; public int AttemptCount { get; set; } public DateTimeOffset ReceivedAtUtc { get; set; } public DateTimeOffset NextAttemptAtUtc { get; set; } public DateTimeOffset? ProcessedAtUtc { get; set; } public string LastError { get; set; } = string.Empty; }
...
public sealed class PairingCodeRecord { public Guid Id { get; set; } public Guid StoreId { get; set; } public Guid CreatedByUserId { get; set; } public Guid? RegisterId { get; set; } public string CodeHash { get; set; } = string.Empty; public DateTimeOffset CreatedAtUtc { get; set; } public DateTimeOffset ExpiresAtUtc { get; set; } public DateTimeOffset? UsedAtUtc { get; set; } }
...
public sealed class SessionRecord { public Guid Id { get; set; } public Guid UserId { get; set; } public Guid? DeviceId { get; set; } public Guid? RegisterId { get; set; } public string TokenHash { get; set; } = string.Empty; public DateTimeOffset CreatedAtUtc { get; set; } public DateTimeOffset ExpiresAtUtc { get; set; } public DateTimeOffset? RevokedAtUtc { get; set; } }
...
public sealed class CustomerRecord { public Guid Id { get; set; } public string Name { get; set; } = string.Empty; public string? Phone { get; set; } public string? Email { get; set; } public string? TaxId { get; set; } public decimal CreditLimit { get; set; } public bool CreditEnabled { get; set; } public bool CreditFrozen { get; set; } public bool IsActive { get; set; } public DateTimeOffset CreatedAtUtc { get; set; } }
