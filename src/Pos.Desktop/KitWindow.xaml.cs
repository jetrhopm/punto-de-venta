...
public partial class KitWindow : Window
{
    ...
    { if (_kit is null || _components.Count == 0) { StatusText.Text = "Selecciona el kit y agrega componentes."; return; } Client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", SessionContext.AccessToken); try { var response = await Client.PostAsJsonAsync("/api/kits", new { kitProductId = _kit.Id, components = _components.Select(item => new { componentProductId = item.Id, quantity = item.Quantity }).ToArray() }); response.EnsureSuccessStatusCode(); StatusText.Text = "Kit guardado correctamente."; } catch (Exception exception) { StatusText.Text = $"No se pudo guardar: {exception.Message}"; } }
