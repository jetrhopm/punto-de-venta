...
public partial class ReturnSaleWindow : Window
{
    ...
    private async Task LoadLinesAsync()
    {
        ...
        catch (HttpRequestException) { MessageText.Text = "No se pudieron cargar las partidas."; }
    }
    private void OnLineSelected(object sender, MouseButtonEventArgs e) { if (LinesList.SelectedItem is LineRow row) { _selected = row; SelectedText.Text = $"{row.Line.Description} | Disponible: {row.Line.SoldQuantity - row.Line.ReturnedQuantity:0.###}"; QuantityTextBox.Text = "1"; } }
    private async void OnReturnClick(object sender, RoutedEventArgs e)
    {
        if (_selected is null || !TryParse(QuantityTextBox.Text, out var quantity) || quantity <= 0m || string.IsNullOrWhiteSpace(ReasonTextBox.Text)) { MessageText.Text = "Selecciona partida, cantidad positiva y motivo."; return; }
