...
public partial class ProductCatalogWindow : UserControl
{
    ...
    private void OnKitsClick(object sender, RoutedEventArgs e) { new KitWindow(_departments.Select(item => new ProductEditorWindow.DepartmentOption(item.Id, item.Name)).ToArray(), _configuredWeightUnit, _autoPriceWithProfit, _defaultProfitPercent) { Owner = Window.GetWindow(this) }.ShowDialog(); }
