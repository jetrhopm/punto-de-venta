...
public partial class CutSettingsWindow : Window
{
    ...
    private async void OnLoaded(object sender, RoutedEventArgs e)
    {
        try
        {
            var settings = await ApiClient.Client.GetFromJsonAsync<CutSettings>("api/cut-settings") ?? new(true, true, false, false, 0m, string.Empty);
            ...
            WarnCashLimitOption.IsChecked = !settings.BlockSalesWhenCashLimitReached; BlockCashLimitOption.IsChecked = settings.BlockSalesWhenCashLimitReached;
    ...
    private void RefreshEnabledState() { var enabled = CashLimitEnabledBox?.IsChecked == true; AutoAdjustBox.IsEnabled = CountAndAdjustOption?.IsChecked == true; CashLimitTextBox.IsEnabled = enabled; CashLimitMessageTextBox.IsEnabled = enabled; WarnCashLimitOption.IsEnabled = enabled; BlockCashLimitOption.IsEnabled = enabled; }
    private async void OnSaveClick(object sender, RoutedEventArgs e)
    {
        ...
        var command = new { requireCashCountOnClose = CountAndAdjustOption.IsChecked == true, autoAdjustCashDifference = AutoAdjustBox.IsChecked == true, cashLimitEnabled = CashLimitEnabledBox.IsChecked == true, blockSalesWhenCashLimitReached = BlockCashLimitOption.IsChecked == true, cashLimit = limit, cashLimitMessage = CashLimitMessageTextBox.Text };
        try
        {
            using var response = await ApiClient.Client.PutAsJsonAsync("api/cut-settings", command);
            if (!response.IsSuccessStatusCode)
            {
                StatusText.Text = await ConfigurationFeedback.ReadErrorAsync(response, "No se pudo guardar la configuración de corte. Revisa los valores e inténtalo de nuevo.");
                return;
            }
            ConfigurationFeedback.ShowSavedAndClose(this, "Corte de caja", "Las reglas se usarán en los próximos cierres de turno.");
        }
        catch (Exception exception) { StatusText.Text = ConnectionHelp.FromException(exception, "No se pudo guardar la configuración"); }
    }
    private sealed record CutSettings(bool RequireCashCountOnClose, bool AutoAdjustCashDifference, bool CashLimitEnabled, bool BlockSalesWhenCashLimitReached, decimal CashLimit, string CashLimitMessage);
}
